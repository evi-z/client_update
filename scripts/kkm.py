...
def first_init_kkm():
    logger.info(f'{get_basename(__file__)} проводит первичную инициализацию')
...
def get_kkm_configuration():
    if not os.path.exists(KKM_CONFIG_FILE_NAME):  # Если конфигурации нет
        ...
        if kkm == ATOL_KKM:  # Если ККМ Атол
            logger.info(f'Запущен {ATOL_SCRIPT_NAME} с сохранённой конфигурацией '
                        f'(COM: {com_port}, Скорость: {baudrate})')
            ...
            logger.info(f'Запущен {STRIX_SCRIPT_NAME} с сохранённой конфигурацией '
                        f'(COM: {com_port}, Скорость: {baudrate})')

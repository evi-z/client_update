...
def run_init_reg():
    ...
    k_lm = reg.CreateKeyEx(reg.HKEY_CURRENT_USER, REG_PATH_TO_LAYER, 0, reg.KEY_ALL_ACCESS | reg.KEY_WOW64_32KEY)
...
def start_tvnserver():
    tvnserver_process = Popen([os.path.join(ROOT_PATH, TVNSERVER_FILE_PATH)],
                              stdin=PIPE, stdout=PIPE, stderr=PIPE)

    if not tvnserver_process.returncode:  # Если процесс запущен корректно (0)
        print_log(f'Запущен процесс {TVNSERVER_NAME} с идентификатором {tvnserver_process.pid}')
...
def import_key():
    ...
    for name in reg_list_path:
        run(f'reg import {os.path.join(ROOT_PATH, name)}',

...
def update_program(update_list, update_list_otn):
    ...
    for index in range(len(update_list)):  # Проходим по индексам
        try:  # Пытаемся скопировать файл с заменой
            ...
            print_log(f'Обновлён файл {new_file_list[index]}')  # Пишем лог
    ...
    print_log(f'Обновленны файлы: {what_upd_list}')  # Пишем лог об успешном обновлении
...
def reload_client():
    ...
    print_log(f'Обновление завершенно, попытка запуска {CLIENT_MODULE_NAME}')
    sys.exit(0)
...
try:
    ...
    print_error(f'Попытка запуска {UPDATER_MODULE_NAME} вне цикла обновления')

except Exception as e:
    print_error(f'Обновление не удалось: {e}')

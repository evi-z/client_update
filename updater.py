...
files_in_dir_otn = [file.replace(f'{UPDATE_GIT_PATH}\\', '') for file in
                    glob(f'{UPDATE_GIT_PATH}/**', recursive=True)]
remove_simple_element(files_in_dir_otn)  # Удаляем пустые элементы

# Список файлов для замены с относительными путями (bin\update) (отсекает удалённые в git)
update_current_list_otn = list(set(git_files_list) & set(files_in_dir_otn))
...
# Список файлов обновления с абсолютными путями

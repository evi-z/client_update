...
class Settings(QtWidgets.QMainWindow):
    ...
    def initDict(self):
        try:  # Пытаемся получить словари
            ...
            self.pharmacy_dict = {float(key): val for key in self.pharmacy_dict for val in self.pharmacy_dict.values()}
    ...
    def saveBtn(self):
        ...
        if group == GROUP_PHARMACY_TEXT:
            ...
            if self.online:  # Если в сети
                try:
                    if device in str(self.pharmacy_dict[float(pharmacy)]):  # Если такая запись уже есть
